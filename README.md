# ACTIVITAT_9
## Proves body-field
### Importar Field
![Captura importar Field](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_field/import_Field.png)

### Editar atribut de preu
![Captura codi main i atribut preu](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_field/body_field.png)

### Error i missatge en preu 0
![Captura error i missatge](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_field/valida_field.png)

### Metode:Put / Codi:200
![Captura Metode Put amb codi de sortida 200](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_field/put_200_postman.png)

## Proves body-nested-models
### Importar HttpUrl per atribut de la subclase
![Captura import HttpUrl ](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_nested_models/import_HttpUrl.png)

### Crear la sublase Image
![Captura sub clase Image](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_nested_models/sub_clase_image.png)

### Editar la clase amb una llista de subclase
![Captura edit Item](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_nested_models/clase_item_subclase_image.png)

### Crear una super clase amb item com atribut
![Captura super clase](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_nested_models/superclase_offer.png)

### Crear funcio amb llista de clases com parametre
![Captura func](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/body_nested_models/func_list_param.png)

## USER_READ
### Estructura arxius
![Estructura arxius](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/read_users/documentacio_9_6.png)

### Arxiu connexio
![Arxiu connexio](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/read_users/documentacio_9_3.png)

### Funcio read_all_users
![Funcio read_all_users](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/read_users/documentacio_9_7.png)

### Schema per usuari
![Schema per usuari](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/read_users/documentacio_9_4.png)

### Arxiu main
![Arxiu main](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/read_users/documentacio_9_5.png)

### Postman codi:200
![Postman codi:200](https://github.com/luisacio/UF2_FASTAPI/blob/activitat_9/ACTIVITAT_9/IMATGES/read_users/documentacio_9_1.png)


